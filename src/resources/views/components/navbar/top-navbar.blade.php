...
<nav class="navbar navbar-expand-lg" style="background-color: var(--bs-primary);">
    <div class="{{ $__data['sleek::fluid'] ?? true ? 'container-fluid' : 'container' }}">
        ...
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#sleek-navbar"
                aria-controls="sleek-navbar" aria-expanded="false" aria-label="Toggle navigation">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="navbarDropdown-{{ $key }}">
                                @foreach($navItem['items'] as $dropdownItem)
                                    <li><a class="dropdown-item"
                                           href="{{ $dropdownItem['route'] }}">{{ $dropdownItem['label'] }}</a></li>
                                @endforeach
                            </ul>
                        </li>
                    @else
                        <!-- Normales Nav Item -->
                        <li class="nav-item">
                            <a class="nav-link {{ request()->url() === $navItem['route'] ? 'active' : '' }}"
                               href="{{ $navItem['route'] }}">{{ $navItem['label'] }}</a>
                        </li>
                    @endif
                @endforeach
            </ul>
                ...
                @unless(($__data['sleek::authentication'] ?? null) === false)
                    <ul class="navbar-nav ms-auto mb-2 mb-lg-0">
                        @if(Auth::check())
                            <li class="nav-item">
                                <a class="nav-link"
                                   href="{{ $__data['sleek::authentication']['logout'] ?? route('logout') }}">
                        ...
                        @else
                            <li class="nav-item">
                                <a class="nav-link"
                                   href="{{ $__data['sleek::authentication']['login'] ?? route('login') }}">
